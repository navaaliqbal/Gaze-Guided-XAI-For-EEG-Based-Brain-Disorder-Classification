...
class EDFPlotCanvas(FigureCanvas):
    ...
    def load_edf(self, file_path):
        ...
        try:
            ...
            # Remove all figure margins so axes fill the window
            # self.fig.subplots_adjust(left=0, right=1, top=1, bottom=0)
            # Leave margin at the bottom for time axis
            # self.fig.subplots_adjust(left=0.05, right=0.98, top=0.95, bottom=0.1)
            # self.fig.subplots_adjust(left=0.03, right=0.98, top=0.95, bottom=0.05)
    ...
    def plot_current_window(self):
        ...
        offset_step = 100  # Fixed vertical offset for clinical-style stacking
        ...
        for i, label in enumerate(self.labels):
            ...
            self.ax.plot(times, scaled_segment + offset, color='black') 
            # //nmt

            label_x = self.current_time + self.time_window * 0.01  # just inside the visible range
            self.ax.text(label_x, offset, label, va='center', ha='left', fontsize=9, fontweight='bold',
                        bbox=dict(facecolor='white', alpha=0.9))

        # Draw scale bar (50 µV, 1s)
