...
class EEGGazeFixationDataset(Dataset):
    ...
    def _convert_fixations_to_gaze_map(self, 
        ...
        # gaze_map = gaze_map * 0.9 + 0.1
            ...
            # CORRECT NORMALIZATION: Spatio-temporal blocks
        time_block_size = 500  # 10 seconds at 50Hz
        normalized = np.zeros_like(gaze_map)
        
        for block_start in range(0, n_time, time_block_size):
            block_end = min(block_start + time_block_size, n_time)
            time_block = gaze_map[:, block_start:block_end]
            
            if time_block.max() > 0:
                normalized[:, block_start:block_end] = time_block / time_block.max()
