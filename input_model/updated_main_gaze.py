...
def main(lr=1e-4, epochs=50, batch_size=32, accum_iter=1, 
    ...
    for epoch in range(hyps['epochs']):
        ...
        eval_stats, ev_labels, ev_preds, ev_files = evaluate_model_comprehensive(
            model, eval_loader, device,gaze_weight=gaze_weight,gaze_loss_type=gaze_loss_type,
            gaze_loss_scale=gaze_loss_scale,
            stats_tracker=stats_tracker, dataset_name="eval"
        )
        ...
        if eval_stats['loss'] < best_loss:
            ...
            torch.save({
                ...
                'gaze_loss_scale': gaze_loss_scale,  # Save the scale   
                'gaze_loss_type': gaze_loss_type,
            }, checkpoint_path)
            print(f"  ✓ Saved best model checkpoint (val_acc: {best_loss:.2f}%, epoch: {epoch+1})")
