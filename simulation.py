...
class EEGViewerWithSimulatedGaze:
    ...
    def __init__(self, edf_path, events_data, time_window=5.0, overlap_ratio=0.2,
        ...
        plt.show(block=True)
    def generate_probabilistic_gaze_points(self, events_in_window):
        """Generate gaze points probabilistically for events in the current window"""
        gaze_data = []

        for event in events_in_window:
            num_channels = len(event['channels'])
            event_duration = max(0.01, event['duration'])
            
            # Base number of points proportional to duration and number of channels
            base_points = int(event_duration * 50 * num_channels)  # 50 pts per sec per channel
            base_points = max(base_points, 5)  # minimum points

            for i in range(base_points):
                # Probabilistic time: Gaussian centered in the middle of event
                time_offset = np.random.normal(event_duration / 2, event_duration / 4)
                time_offset = np.clip(time_offset, 0, event_duration)
                gaze_time = event['start'] + time_offset

                # Only include if within current window
                if self.current_time <= gaze_time <= self.current_time + self.time_window:
                    # Probabilistically pick a channel (more channels = higher chance for each)
                    chosen_channel = np.random.choice(event['channels'])
                    if chosen_channel in self.labels:
                        channel_idx = self.labels.index(chosen_channel)
                        channel_offset = channel_idx * self.offset_step
                        
                        # Add natural vertical variation
                        y_variation = np.random.normal(0, self.offset_step / 6)
                        y_pos = channel_offset + y_variation
                        
                        # Horizontal jitter
                        x_jitter = np.random.normal(0, 0.05)

                        # Duration/intensity can also vary randomly
                        gaze_duration = np.random.exponential(scale=0.2)  # probabilistic small duration
    ...
    def generate_gaze_points_for_events(self, events_in_window):
        """Generate gaze points ONLY for events in the current window"""
        gaze_data = []
        
        for event in events_in_window:
            # Calculate how many gaze points based on event duration
            num_points = max(10, int(event['duration'] * 50))  # More points for longer events
            
            for i in range(num_points):
                # Time within event duration (biased toward center)
                ...
                # Only include if within current window
                if self.current_time <= gaze_time <= self.current_time + self.time_window:
                    # Choose a random channel from the event's channels
                    chosen_channel = random.choice(event['channels'])
                    
                    # Get channel index and position
                    if chosen_channel in self.labels:
                        channel_idx = self.labels.index(chosen_channel)
                        channel_offset = channel_idx * self.offset_step
    ...
    def create_heatmap_for_events(self, gaze_points, window_height):
        """Create a heatmap ONLY around event gaze points"""
        if not gaze_points:
            return None
    ...
    def plot_current_window(self):
        ...
        gaze_points = self.generate_probabilistic_gaze_points(events_in_window)

        print(f"Generated {len(gaze_points)} gaze points for events")
        
        # === Create and plot heatmap ONLY for event gaze points ===
...
if __name__ == "__main__":
    # Load your events data from CSVC
    events_df = pd.read_csv(r"D:\simulation\6.csv")
    ...
    EEGViewerWithSimulatedGaze(
        edf_path=r"D:\simulation\0000006.edf",
