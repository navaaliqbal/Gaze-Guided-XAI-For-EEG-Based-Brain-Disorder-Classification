...
# ========== EXISTING ARCHITECTURE COMPONENTS (ONLY ResConv UPDATED) ==========
...
class ResConv(nn.Module):
    def __init__(self, in_channels, dropout_rate=0.5):
        ...
        self.conv1 = nn.Conv1d(in_channels=in_channels, out_channels=8, 
                              kernel_size=5, padding=2)
        self.bn1 = nn.BatchNorm1d(8)
        self.dropout1 = nn.Dropout1d(dropout_rate)  # ADDED DROPOUT HERE
        self.conv2 = nn.Conv1d(in_channels=in_channels + 8, out_channels=16, 
                              kernel_size=5, padding=2)
        self.bn2 = nn.BatchNorm1d(16)
        self.dropout2 = nn.Dropout1d(dropout_rate)  # ADDED DROPOUT HERE
    ...
    def forward(self, input):
        x1 = F.relu(self.bn1(self.conv1(input)))
        x1 = self.dropout1(x1)  # APPLY DROPOUT HERE
        x1 = torch.cat((x1, input), dim=1)
        x2 = F.relu(self.bn2(self.conv2(x1)))
        x2 = self.dropout2(x2)  # APPLY DROPOUT HERE
        return torch.cat((x2, x1), dim=1)
...
class NeuroGATE_Gaze_MultiRes_Simple(nn.Module):
    ...
    def __init__(self, n_chan: int = 22, n_outputs: int = 2, 
                 original_time_length: int = 15000, dropout_rate: float = 0.5):
        super().__init__()
        self.model = NeuroGATE_Gaze_MultiRes(n_chan, n_outputs, original_time_length, dropout_rate)
